...
function calcular(){    
    ...
    if(a==0 || b==0 || c==0){
        window.alert("ingreso un valor cero y es incorrecto");
    }
    ...
    if(raiz>=0){
        ...
        resultado = "<b>"+ "El discriminante es menor que cero por ende la ecuación no tiene resolución real." + "</b>";
    }
...
function evaluar(){
    ...
    if(fin<ini) return window.alert("ingreso mal los intervalos");
...
function drawBasic(intervalo, resultados) {
    ...
    for (let i = 0; i < intervalo.length; i++) {
        datos.push([intervalo[i], resultados[i]]);
    }

    data.addRows(datos);

    var options = {
        title: "Grafica función cuadratica",
        height:400,
        width:500,
        hAxis: {
            title: 'X'
